...
var d3Graphs = {
    ...
    showHud: function() {
        ...
        $("#handle").draggable({
            axis: 'x',
            containment: "parent",
            grid: [this.handleInterval, this.handleInterval],
        ...
        $("#hudButtons .countryTextInput").autocomplete({
            source: selectableCountries,
            autoFocus: true
        });
    ...
    clickTimeline: function() {
        ...
        //计算偏移量
        var index1 = $('#history ul li').index(this);
        console.log('第几个');
        console.log(index1);
        var year = $(this).html();
    ...
    positionHistory: function(windowWidth) {
        ...
        $("#graphIcon").css('bottom', 300 + 'px');
    },
    countryFocus: function(event) {
        //console.log("focus");
        setTimeout(function() {
            $('#hudButtons .countryTextInput').select()
        }, 50);
    },
    ...
    updateViz: function(country) {
        ...
        console.log('yearOffset%12');
        console.log(yearOffset);

        if (yearOffset >= 0 && yearOffset <= 11) {
            var year = yearOffset + kyear;
        } else {
            //从2014 开始 增加年份与月份
            var year = 100 * Math.floor(yearOffset / 12) + kyear + (yearOffset % 12);
        }
    ...
    drawHistogram: function() {
        ...
        var activeYearData = [{
            x: yearOffset,
            y: activeYearImports != null ? activeYearImports.y : -1,
            max: maxVal,
            show: activeYearImports != null,
            type: "imports"
        }, {
            x: yearOffset,
            y: activeYearExports != null ? activeYearExports.y : -1,
            max: maxVal,
            show: activeYearExports != null,
            type: 'exports'
        }];
        ...
        yearDotLabels.attr('x', function(d) {
                return d3Graphs.histogramLeftPadding + d3Graphs.histogramXScale(d.x);
            })
            .attr('y', function(d) {
    ...
    drawBarGraph: function() {
        ...
        for (var i = 0; i < importLabelArray.length; i++) {
            ...
            if (pieceHeight < smallLabelSize) {
                ...
                var weaponLabel = importLabel.append('text').text('人数').attr('text-anchor', 'end').attr('y', 21)
        ...
        for (var i = 0; i < exportLabelArray.length; i++) {
            ...
            if (pieceHeight < smallLabelSize) {
                ...
                var textLabel = exportLabel.append('text').text(function(d) {
                    ...
                    console.log(d.type);
                    return reverseWeaponLookup[d.type].split(' ')[0].toUpperCase();
                }).attr('text-anchor', 'start').attr('y', 15).attr('class', function(d) {
                    return 'export ' + d.type
                });
                labelHeight = fontSizeInterpolater((data.amount - minImExAmount) / (maxImExAmount - minImExAmount));
                labelBGYPos = -labelHeight;
                ...
                var textLabel = exportLabel.append('text').text(function(d) {
                    ...
                    console.log(d.type);
                    return reverseWeaponLookup[d.type].split(' ')[0].toUpperCase();
                }).attr('text-anchor', 'start').attr('y', 8).attr('class', function(d) {
                    return 'export ' + d.type
                });
                var weaponLabel = exportLabel.append('text').text('人数').attr('text-anchor', 'start').attr('y', 21)
                    .attr('class', function(d) {
                        return 'export ' + d.type
                    });
                labelHeight = fontSizeInterpolater((data.amount - minImExAmount) / (maxImExAmount - minImExAmount));
