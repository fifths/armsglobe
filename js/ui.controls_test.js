...
var d3Graphs = {
    ...
    showHud: function() {
        ...
        $("#hudHeader .countryTextInput").autocomplete({
    ...
    countryFocus: function(event) {
        console.log("focus");
        setTimeout(function() {
            $('#hudHeader .countryTextInput').select()
        }, 50);
    },
    ...
    drawHistogram: function() {
        ...
        var activeYearData = [{
            x: yearOffset,
            y: this.histogramImportArray[yearOffset],
            max: maxVal
        }, {
            x: yearOffset,
            y: this.histogramExportArray[yearOffset],
            max: maxVal
        }];
    ...
    drawBarGraph: function() {
        ...
        for (var type in reverseWeaponLookup) {
            importArray.push({
                "type": type,
                "amount": selectedCountry.summary.imported[type]
            });
            exportArray.push({
                "type": type,
                "amount": selectedCountry.summary.exported[type]
            });
        }
        ...
        exportLabels.append('text').text(function(d) {
            return reverseWeaponLookup[d.type].split(' ')[0].toUpperCase();
        }).attr('y', 15).attr('class', function(d) {
            return 'export ' + d.type
        });
        exportLabels.append('text').text(function(d) {
            return abbreviateNumber(d.amount);
